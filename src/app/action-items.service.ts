...
export class ActionItemsService {
  ...
  getActionItem(id: string): Observable<ActionItem> {
    return fakeNetworkRequest(this.actionItems$.value.find(it => it.id === id))
  }
  ...
  updateActionItem(
    ...
    this.updateActionItems(this.actionItems.map(item => {
      if (item.id !== id) {
        return item
      }
      ...
      if (update.assigneeId !== undefined) {
        updatedActionItem.assignee = this.users.find(it => it.id === update.assigneeId)
      }
  ...
  deleteActionItem(id: string) {
    this.updateActionItems(this.actionItems.filter(it => it.id === id))
  }
